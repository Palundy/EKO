...
class TMCL:
    ...
    AXIS_PARAMETER_ACTUAL_SPEED = 3
    AXIS_PARAMETER_MAXIMUM_POSITIONING_SPEED = 4
    AXIS_PARAMETER_MAXIMUM_ACCELERATION = 5
    ...
    def __init__(self, port, baudrate = 9600, timeout = 1, write_timeout = 1):
        ...
        print("Succesfully initialized serial connection.")
    ...
    def actual_speed(self) -> int:
        """
        Wrapper function to send the command
        to get the actual speed of the motor.

        Returns:
            int: The actual speed.
        """

        if self.send_command(
            TMCL.COMMAND_GET_AXIS_PARAMETER,
            0,
            TMCL.AXIS_PARAMETER_MAXIMUM_POSITIONING_SPEED
        ).ok():
            return self.reply_value()
        return None
    

    def set_actual_speed(self, speed: int):
        """
        Wrapper function to send the command
        to set the actual speed.

        Args:
            speed (int): The actual speed.
        ...
        # Send the command to move the
        # speed to the given absolute speed
        return self.send_command(
            TMCL.COMMAND_SET_AXIS_PARAMETER,
            speed,
            TMCL.AXIS_PARAMETER_MAXIMUM_POSITIONING_SPEED
    ...
    def wiggle(self, wiggle_deviation):
        
        # Convert the `wiggle_deviation` into an integer
        wiggle_deviation = int(wiggle_deviation)


        # Deviate to one side
        self.relative_rotation(-wiggle_deviation)

        # Deviate back to the center
        self.relative_rotation(wiggle_deviation)

        # Deviate to the other side
        self.relative_rotation(wiggle_deviation)

        # Deviate to the center
        self.relative_rotation(-wiggle_deviation)
