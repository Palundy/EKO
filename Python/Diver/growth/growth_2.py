...
def explode_genome(genome):
    GT = convert_chromosome(genome[0:4]) / 16*1
    GR = convert_chromosome(genome[4:8]) / 16*2
    GB = convert_chromosome(genome[8:12]) / 16*2
    GL = convert_chromosome(genome[12:16]) / 16*2
    IBFT = convert_chromosome(genome[16:22]) / 64*2
    IBFR = convert_chromosome(genome[28:34]) / 64*2
    IBFB = convert_chromosome(genome[34:40]) / 64*2
    IBFL = convert_chromosome(genome[40:46]) / 64*2
...
def evaluate_organisms(organisms, cells, t):
    organisms[t + 1] = organisms[t]
    cells[t + 1] = cells[t]

    for x in range(x_range):
        for y in range(y_range):
            organism = organisms[t][x][y]
            if (np.all(organism == 0)):
                continue

            GT, GR, GB, GL, IBFT, IBFR, IBFB, IBFL = explode_genome(organism)

            # Check whether the top part gets irradiated
            T_free = np.any(cells[t, x, y:] == 1)

            # Calculate chance to grow TOP
            rand = random.random()
            if (GT > rand) or (GT/IBFT > rand and T_free):
                # Grow TOP
                if (y != y_range-1 and cells[t+1][x][y+1] == 0):
                    organisms[t+1][x][y+1] = organism
                    cells[t+1][x][y+1] = 1

            # Calculate chance to grow RIGHT
            if (GR > rand):
                # Grow RIGHT
                if (x != x_range-1 and cells[t+1][x+1][y] == 0):
                    organisms[t+1][x+1][y] = organism
                    cells[t+1][x+1][y] = 1

            # Calculate chance to grow BOTTOM
            if (GB > rand):
                # Grow BOTTOM
                if (y != 0 and cells[t+1][x][y-1] == 0):
                    organisms[t+1][x][y-1] = organism
                    cells[t+1][x][y-1] = 1
